...
- Separate Camera-family entries for GoPro, Insta360 GPS Remote, Insta360 Mini
  ...
  but retain their captured advertising identities, shutter vectors, and
  camera-state formats. Both include guarded shutdown and ORBIT wake. DJI
  ...
  Remote's X6 power behavior is operator-confirmed, while Ble(e)p's split Mini
  implementation still awaits a flashed-panel lifecycle test. Google Pixel 9
...
| GoPro MAX2 (Open GoPro BLE) | Supported | Bonded pairing, explicit Start/Stop, confirmed recording state, Sleep, physical wake, and post-boot Ready are operator-confirmed on the panel. Other models and broader coexistence coverage remain open. |
...
| Insta360 X3 | Supported | Operation through Ble(e)p's GPS Remote path is operator-approved. Model-specific capture, full reconnect/power coverage, and coexistence remain open. |
...
| Insta360 X6 | Experimental candidate | Mini Remote advertising, shutter toggle, and video state are capture-backed. The physical Mini Remote's Start/Stop and power-off/on behavior are operator-confirmed; Ble(e)p's Mini path still needs a panel test. |
...
| Aputure Light — amaran Ray 60c / Ace 25c; Aputure MC Pro / MT Pro | Supported exact models | Automatically selects one stable candidate, or shows a bounded picker when multiple factory-reset Aputure/amaran fixtures are nearby, before provisioning onto the panel-owned mesh. Per-node power/look routing follows the working Studio Lighter unicast path. MT Pro is currently mislabeled as MC Pro because both report the known `0x03F6:0x1000` vendor tuple and the same composition header IDs. Pano 60c/120c, broader physical selection, multi-fixture isolation, recovery, and soak gates remain open. |
