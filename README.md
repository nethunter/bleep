...
| Aputure Light | Experimental | Automatically selects one stable candidate, or shows a bounded picker when multiple factory-reset Aputure/amaran fixtures are nearby, before provisioning onto the panel-owned mesh. Per-node power/look routing follows the working Studio Lighter unicast path. Ace 25c, Pano 60c, Pano 120c, and MC Pro identification is implemented; physical selection, four-fixture isolation, recovery, and soak gates remain open. |
